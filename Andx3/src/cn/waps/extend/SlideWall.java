...
public class SlideWall {
	...
	//自定义的抽屉（获取该对象时，common_waps_demo_slidewall.xml中使用的抽屉对象的名称，必须和SlideWallDrawer类的全限定类名匹配）
	...
	public LinearLayout getView(final Context context, int handleMarginLeft, int itemWidth, int itemHeight){
		...
		try {
			// 其中获取id的方式相当于 R.layout.common_waps_demo_slidewall
			// common_waps_demo_slidewall.xml布局文件的id
			int layout_id = context.getResources().getIdentifier("common_waps_demo_slidewall", "layout", context.getPackageName());
			// common_waps_demo_slidewall.xml布局文件中最外层布局的id
